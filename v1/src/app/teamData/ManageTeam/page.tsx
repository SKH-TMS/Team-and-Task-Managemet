...
export default function ManageTeam() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const response = await fetch("../../api/teamData/getTeams", {
          method: "GET",
  ...
  const handleTeamChange = async (teamId: string) => {
    ...
    setSelectedProject(null); // Reset project selection when team changes
    try {
      // Fetch projects assigned to the selected team
      const response = await fetch(`/api/teamData/getProjects`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleProjectChange = (projectId: string) => {
    setSelectedProject(projectId); // Set the selected project
  };
  ...
  const handleTaskAssign = async () => {
    if (!selectedTeam || !selectedProject || !selectedDeadlineDate) {
