...
export default function ProjectTasksPage() {
  ...
  const handleTaskClick = (task: Task) => {
    // Redirect to a new page with task details
    router.push(`/teamData/teamLeaderData/EditTask/${task.TaskId}`);
  };
  ...
  // Assign bg color based on whether the task is selected
  const getTaskBgColor = (taskId: string) => {
    return selectedTaskIds.includes(taskId) ? "bg-pink-100" : ""; // Pink when selected
  };
